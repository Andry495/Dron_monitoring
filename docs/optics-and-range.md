...
Версия: 0.1 · 2026-07-08

Цели: **0.3–8 m** (дрон ~30–50 cm … самолёт/вертолёт до ~8 m).  
Роли камер:
- **Небо (купол / 4× фикс)** — только факт движения + грубое направление `(az, el)`.
- **Угловые PTZ** — наведение, зум, форма/класс, **координаты** (триангуляция).
...
|--------|-----------------|--------|
| Движение / blob | **≥ 4–8** | Небо (широкий обзор) |
| Детектор NN (купол) | **≥ 12–16** | Небо |
...
### Небо — 4× фикс 2.8 mm (основной вариант)
...
### Небо — 1× fisheye 12 MP (альтернатива)
...
```mermaid
sequenceDiagram
    participant Sky as Небо (широкий)
    participant Core as Ядро
    participant PTZ as Угловая PTZ

    Sky->>Core: движение, грубый az/el
    Core->>PTZ: goto + zoom auto
    PTZ->>Core: кадр 25×, bbox
    Core->>Core: класс + уточнённые углы
...
| | Готовая PTZ | DIY + энкодер |
|---|-------------|---------------|
| Углы из коробки | Да (ONVIF) | Да (MQTT + AS5600) |
| Повторяемость | Средняя (люфт) | Высокая (если механика металл) |
| Зум + FOV | Заводская кривая | Своя калибровка bullet |
| Для триангуляции | **Достаточно** после калибровки | Потенциально точнее |
| Стоимость | Выше | Ниже железо, выше время |
...
См. также: [cameras.md](./cameras.md), [architecture.md](./architecture.md).
