...
Турель **не заменяет** PTZ куба: куб обнаруживает и даёт геометрию; турель — **точное наведение и выстрел** в ближней зоне.
...
```text
                    цель ✈
                      ╲
                       ╲  R ≤ 50 m
                        ╲
    [куб наблюдения]     ●━━━━━━► траектория с упреждением
         ▲               │
         │               ▼
    Sky + PTZ      ┌─────────────┐
                   │  ТУРЕЛЬ     │
...
Рекомендуемое размещение: **центр площадки** или смещение ≤2 m от куба, чтобы зона 50 m перекрывала подходы к кубу.
...
POST /v1/turret/arm   { "enabled": true, "operator_token": "..." }
POST /v1/turret/safe  { "reason": "manual" }

GET  /v1/turret/status
→ state, az, el, pressure_bar, rounds_loaded, faults[]
```
